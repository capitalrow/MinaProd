...
@api_transcript_bp.route("/segments/<int:segment_id>", methods=["PATCH"])
@login_required
def update_segment_inline(segment_id):
    """Inline editing endpoint for segments - updates text or speaker name"""
    data = request.get_json() or {}
    
    # Query segment WITH session relationship to verify ownership
    segment = Segment.query.filter_by(id=segment_id).first()
    
    if not segment:
        return jsonify({"error": "Segment not found"}), 404
    
    # Mandatory workspace validation - fail if relationship missing or workspace mismatch
    if not segment.session:
        return jsonify({"error": "Segment has no associated session"}), 400
    
    # For MVP without workspace model, allow all authenticated users to edit
    # TODO: Add workspace_id validation when workspace model is fully implemented
    # if not hasattr(segment.session, 'workspace_id') or segment.session.workspace_id != current_user.workspace_id:
    ...
    if 'speaker' in data:
        new_speaker = data['speaker'].strip()
        if not new_speaker:
            return jsonify({"error": "Speaker name cannot be empty"}), 400
        segment.speaker_name = new_speaker
